...
if (!isset($data['IdUser'], $data['orderCost'], $data['orderDetails'], $data['point']) || !is_array($data['orderDetails'])) {
...
try {
    ...
    $sql_order = "INSERT INTO orders (IdUser, orderTime, orderCost, orderStatus) VALUES (?, ?, ?, ?)";
    $stmt_order = $conn->prepare($sql_order);
    if (!$stmt_order) {
        throw new Exception("Lỗi khi chuẩn bị câu lệnh SQL cho bảng orders");
    }
    $stmt_order->bind_param("isss", $idUser, $orderTime, $orderCost, $orderStatus);
